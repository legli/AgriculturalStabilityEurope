...
## regression models
# stability
modDiversityRegion <- lm(stability~diversity+fertilizer+irrigation+soilQuality+soilDiversity+instabilityTemp+instabilityPrec+timePeriod+
...
modAsynchronyRegion <- lm(stability~asynchrony+fertilizer+irrigation+soilQuality+soilDiversity+instabilityTemp+instabilityPrec+timePeriod+
                           asynchrony:fertilizer+asynchrony:irrigation+asynchrony:soilQuality+asynchrony:soilDiversity+asynchrony:instabilityTemp+asynchrony:instabilityPrec
                         ,data=dfCenterRegion)
summary(modAsynchronyRegion)

modDiversityRegionLME <- lme(stability~diversity+fertilizer+irrigation+soilQuality+soilDiversity+instabilityTemp+instabilityPrec+timePeriod+
                               diversity:fertilizer+diversity:irrigation+diversity:soilQuality+diversity:soilDiversity+diversity:instabilityTemp+diversity:instabilityPrec,
...
modAsynchronyRegionLME <- lme(stability~asynchrony+fertilizer+irrigation+soilQuality+soilDiversity+instabilityTemp+instabilityPrec+timePeriod+
                                asynchrony:fertilizer+asynchrony:irrigation+asynchrony:soilQuality+asynchrony:soilDiversity+asynchrony:instabilityTemp+asynchrony:instabilityPrec,
                               random=~1|Country,method = "ML",
                               data=dfCenterRegion)
...
modDiversityTime <- lme(asynchrony ~ diversity, random=~1+diversity|timePeriod, method="ML",control=lmeControl(opt='optim'),data = dfDiversityAsynchrony)
modDiversityCountryR <- lme(asynchrony ~ diversity, random=~1+diversity|Country, method="ML",control=lmeControl(opt='optim'),data = dfDiversityAsynchrony)
modDiversityLM <- lm(asynchrony ~ diversity,data = dfDiversityAsynchrony)
summary(modDiversityLM)
anova(modDiversityTime,modDiversityLM)  # AIC: -180.9117 -172.6012
anova(modDiversityCountryR,modDiversityLM)  # AIC: -247.2830 -172.6012
...
## check colinearity
round(cor(dfCenterCountry[,2:12],method='s'),2)

## regression models
# stability
modDiversityCountry <- lm(stability~diversity+fertilizer+soilDiversity+instabilityTemp+instabilityPrec+timePeriod,
                          data=dfCenterCountry)
summary(modDiversityCountry)
vif(modDiversityCountry)

modAsynchronyCountry <- lm(stability~asynchrony+fertilizer+soilDiversity+instabilityTemp+instabilityPrec+timePeriod,
                           data=dfCenterCountry)
...
dfCombined <- dfCombined[unlist(lapply(1:8,function(i)seq(i,16,8))),]
dfText <- data.frame(xpos=sort(c(1:8-0.22,1:8+0.22)),ypos=dfCombined$labHeight,lab=dfCombined$lab,Model=dfCombined$Model)
...
Table1a <- funTables(modDiversityRegionLME,2,F, 
                      c("(Intercept)","Diversity","sqrt(Nitrogen use)","Irrigation","Soil productivity","Soil diversity","Temperature instability","Precipitation instability","Time",
                         "Diversity:sqrt(Nitrogen use)","Diversity:Irrigation","Diversity:Soil productivity","Diversity:Soil diversity","Diversity:Temperature instability","Diversity:Precipitation instability"))
Table1b <- funTables(modAsynchronyRegionLME,2,F,                      
                     c("(Intercept)","Asynchrony","sqrt(Nitrogen use)","Irrigation","Soil productivity","Soil diversity","Temperature instability","Precipitation instability","Time",
                         "Asynchrony:sqrt(Nitrogen use)","Asynchrony:Irrigation","Asynchrony:Soil productivity","Asynchrony:Soil diversity","Asynchrony:Temperature instability","Asynchrony:Precipitation instability"))

write.xlsx(cbind(Table1a,Table1b),"results/Table1.xlsx")

# national
TableS1a <- funTables(modDiversityCountry,2,T, 
                     c("(Intercept)","Diversity","sqrt(Nitrogen use)","Soil diversity","Temperature instability","Precipitation instability","Time"))
TableS1b <- funTables(modAsynchronyCountry,2,T,                      
                     c("(Intercept)","Asynchrony","sqrt(Nitrogen use)","Soil diversity","Temperature instability","Precipitation instability","Time"))
