...
# save target crops
write.xlsx(dfCalories[order(dfCalories[,"NameFAO"],dfCalories[,"Name"]),c("NameFAO","Name")],"datasetsDerived/cropsFinal.xlsx")
...
dfProduction$ISO2 <- substr(dfProduction$Region,1,2)
...
#### Suitability
# regional level
dfSuitability <- read.csv("datasetsDerived/suitability_regional.csv")
head(dfSuitability)

# country level
dfSuitabilityCountry <- read.csv("datasetsDerived/suitability_national.csv")
head(dfSuitabilityCountry)

# derive fao crop areas (to weight suitability)
dfAreaHarvestedFao <- aggregate(AreaHarvested~Region+NameFAO+Year,dfProduction,sum)
dfAreaHarvestedFaoCountry <- aggregate(AreaHarvested~Country+NameFAO+Year,dfProduction,sum)
...
vecLevelFinal <- Reduce(intersect,list(dfProduction$Region,dfProductionSum$Region,dfShannon$Region,dfTemp$Region,dfPrec$Region,dfSuitability$Region,dfSoil$Region,dfAreaHarvestedFao$Region))
...
lsAll <- lapply(vecLevelFinal,function(reg){
  ...
  lsAggregate <- lapply(c(1978,1988,1998,2008),function(yearStart){
    ...
    if(sumRegion==10){  
      ...
      dfSuitabilityLevel <- dfSuitability[which(dfSuitability$Region==reg),]
      dfAreaHarvestedFaoLevel <- aggregate(AreaHarvested~NameFAO,dfAreaHarvestedFao[which(dfAreaHarvestedFao$Region==reg&dfAreaHarvestedFao$Year>=yearStart&dfAreaHarvestedFao$Year<=(yearStart+9)),],mean)
      dfAreaHarvestedFaoLevel <- merge(dfAreaHarvestedFaoLevel,dfSuitabilityLevel,by="NameFAO")
      ...
      dfSummary$soil <- mean(dfSoilLevel$soilQuality,na.rm=T)
      dfSummary$suitability <- weighted.mean(dfAreaHarvestedFaoLevel$suitability,dfAreaHarvestedFaoLevel$AreaHarvested)
...
## omit NA
dfAll <- na.omit(dfAll)
length(unique(dfAll$Region)) ## 175 regions
nrow(dfAll) ## 419 data points
...
dfAll <- dfAll[,c("Region","Country","timePeriod",
                  ...
                  "asynchrony","diversity","suitability","soil","fertilizer","irrigation",
...
### national level
# get regions across datasets
vecCountryFinal <- Reduce(intersect,list(dfProductionCountry$Country,dfProductionSumCountry$Country,dfShannonCountry$Country,dfTempCountry$Country,dfPrecCountry$Country,dfSuitabilityCountry$Country,dfSoilCountry$Country,dfAreaHarvestedFaoCountry$Country))
...
lsAllCountry <- lapply(vecCountryFinal,function(lev){
  ...
  lsAggregate <- lapply(c(1978,1988,1998,2008),function(yearStart){
    ...
    if(sumCountry==10){  
      ...
      dfSuitabilityLevel <- dfSuitabilityCountry[which(dfSuitabilityCountry$Country==lev),]
      dfAreaHarvestedFaoLevel <- aggregate(AreaHarvested~NameFAO,dfAreaHarvestedFaoCountry[which(dfAreaHarvestedFaoCountry$Country==lev&dfAreaHarvestedFaoCountry$Year>=yearStart&dfAreaHarvestedFaoCountry$Year<=(yearStart+9)),],mean)
      dfAreaHarvestedFaoLevel <- merge(dfAreaHarvestedFaoLevel,dfSuitabilityLevel,by="NameFAO")
...
length(unique(dfAllCountry$Country)) ## 16 countries
nrow(dfAllCountry) ## 45 data points
