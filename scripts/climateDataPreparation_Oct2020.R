...
mapCountry <- mapCountry[mapCountry@data$Country %in%unique(dfRegion$Country),] # only include European countries
plot(mapCountry)

## cropland mask
rasterCropland <- raster("C:/Users/egli/Nextcloud/Cloud/PhD_Leipzig/Data/cropland/cropland2000AD.asc")
rasterCropland[rasterCropland <= 0.01] <- 0 # remove very small cropland areas (< 1ha)

rasterCropland <- raster::aggregate(rasterCropland,fact=6,fun=sum,na.rm=T) # same resolution as climate data
dfCropland <- as.data.frame(rasterCropland)
dfCropland <- cbind(dfID,dfCropland)
head(dfCropland)
names(dfCropland)[4] <- "cropland"
# only keep cells with cropland
dfCropland <- dfCropland[which(dfCropland$cropland>0),]
...
lsTemp <- lapply(61:117,function(y){ # 61 refers to 1961
  ...
  dfTemp[,3:14] <- dfTemp[,3:14]*dfTemp$cropland*dfTemp[,17:28] # mask by cropland and growing period
...
lsPrec <- lapply(61:117,function(y){ # 61 refers to 1961
  ...
  dfPrec[,3:14] <- dfPrec[,3:14]*dfPrec$cropland*dfPrec[,17:28] # mask by cropland and growing period
...
dfExtractCountryTemp <- dfExtractCountryTemp[,2:59]
...
dfExtractCountryPrec <- dfExtractCountryPrec[,2:59]
